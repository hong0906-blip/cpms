...
(function () {
  ...
  function search() {
    ...
    fetch('?r=ajax/workforce_search&q=' + encodeURIComponent(q), {
      ...
      .then(function (data) {
        ...
        var exact = [];
        for (var i = 0; i < items.length; i++) if (trim(items[i] && items[i].name) === q) exact.push(items[i]);
        if (exact.length === 1) { if (status) status.textContent = exact[0].name + ' 인원을 추가합니다.'; submitWorker(exact[0].id); return; }
        var candidates = exact.length > 1 ? exact : items;
        if (!candidates.length) { if (status) status.textContent = '등록된 인력이 없습니다. 옆의 인원 등록 탭에서 먼저 등록하세요.'; return; }
  ...
  document.addEventListener('click', function (event) {
    ...
    if (target && target.id === 'workforceDuplicateAddButton') {
      ...
      if (!checked) { alert('추가할 인원을 선택하세요.'); return; }
  ...
  document.addEventListener('change', function (event) {
    ...
    for (var i = 0; i < checks.length; i++) if (checks[i] !== target) checks[i].checked = false;
  });
