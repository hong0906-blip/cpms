...
function pm_install_patch_sidebar($sidebarPath)
{
    ...
    $variableBlock = "\n/* CPMS_PUBLIC_MAIL_VARIABLE_START */\n"
        ...
        . "\$publicMailIcon = base_url() . '/assets/img/naver_n_icon.svg?v=20260806_77';\n"
...
$requiredFiles = array(
    ...
    'app/services/PublicMailStorageService.php' => "const VERSION = '1.7.7'",
    ...
    'app/services/PublicMailIndexService.php' => "const VERSION = '1.7.7'",
    'app/services/PublicMailService.php' => "const VERSION = '1.7.7'",
    'app/services/PublicMailWebHelper.php' => 'requireDevelopmentDepartment',
    'app/views/public_mail/index.php' => '20260806_77',
    ...
    'app/views/public_mail/settings.php' => '20260806_77',
    'public/public_mail.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.7',
    'public/public_mail_settings.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.7',
    'public/public_mail_action.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.7',
    'public/public_mail_attachment.php' => '',
    'public/assets/css/public_mail.css' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.7',
    'public/assets/img/naver_n_icon.svg' => '',
    'public/assets/js/public_mail.js' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.7',
...
<div class="wrap">
    <div class="card">
        ...
        <p class="sub">패키지 v1.7.7의 실제 핵심 파일 버전과 메일 목록 색인을 확인합니다. 사이드바만 최신인 상태를 전체 패치 완료로 표시하지 않습니다.</p>
        ...
        <div class="status"><span class="dot <?php echo ($latestInstalled&&$allReady) ? 'on' : ''; ?>"></span><strong>설치 상태: <?php echo ($latestInstalled&&$allReady) ? 'v1.7.7 전체 적용' : ($installed ? '일부 파일 업데이트 필요' : '설치 전'); ?></strong></div><div class="status" style="margin-top:10px"><strong>목록 색인: <?php echo !empty($indexStatus['updated_at']) ? pm_install_h($indexStatus['updated_at']).' · '.number_format(isset($indexStatus['item_count'])?(int)$indexStatus['item_count']:0).'건' : '아직 생성되지 않음'; ?></strong></div>

        <div class="files">
            <?php foreach ($checks as $check): ?>
                <?php $checkClass = $check['status']==='latest'?'ok':($check['status']==='old'?'old':'bad'); ?>
                <?php $checkLabel = $check['status']==='latest'?'v1.7.7 확인':($check['status']==='old'?'이전 버전':'파일 없음'); ?>
