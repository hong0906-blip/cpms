...
function pm_install_patch_sidebar($sidebarPath)
{
    ...
    $variableBlock = "\n/* CPMS_PUBLIC_MAIL_VARIABLE_START */\n"
        ...
        . "\$publicMailIcon = base_url() . '/assets/img/naver_n_icon.svg?v=20260806_712';\n"
...
$requiredFiles = array(
    ...
    'app/services/PublicMailService.php' => "const VERSION = '1.7.12'",
    'app/services/PublicMailWebHelper.php' => 'requireDevelopmentDepartment',
    'app/views/public_mail/index.php' => '20260806_712',
    ...
    'app/views/public_mail/settings.php' => '20260806_712',
    'public/public_mail.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.12',
    'public/public_mail_settings.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.12',
    ...
    'public/assets/js/public_mail.js' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.12',
...
<div class="wrap">
    <div class="card">
        ...
        <p class="sub">v1.7.12 속도 복구·원본 제목 재수집 패치입니다. 평상시 화면의 전체 제목 검사를 제거하고 제목 헤더만 100건씩 가져옵니다.</p>
        ...
        <div class="status"><span class="dot <?php echo ($latestInstalled&&$allReady) ? 'on' : ''; ?>"></span><strong>설치 상태: <?php echo ($latestInstalled&&$allReady) ? 'v1.7.12 전체 적용' : ($installed ? '일부 파일 업데이트 필요' : '설치 전'); ?></strong></div><div class="status" style="margin-top:10px"><strong>목록 색인: <?php echo !empty($indexStatus['updated_at']) ? pm_install_h($indexStatus['updated_at']).' · '.number_format(isset($indexStatus['item_count'])?(int)$indexStatus['item_count']:0).'건' : '아직 생성되지 않음'; ?></strong></div>

        <div class="files">
            <?php foreach ($checks as $check): ?>
                <?php $checkClass = $check['status']==='latest'?'ok':($check['status']==='old'?'old':'bad'); ?>
                <?php $checkLabel = $check['status']==='latest'?'v1.7.12 기반 확인':($check['status']==='old'?'이전 버전':'파일 없음'); ?>
