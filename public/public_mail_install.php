...
function pm_install_patch_sidebar($sidebarPath)
{
    ...
    $variableBlock = "\n/* CPMS_PUBLIC_MAIL_VARIABLE_START */\n"
        ...
        . "\$publicMailIcon = base_url() . '/assets/img/naver_n_icon.svg?v=20260806_713';\n"
    ...
    try {
        ...
        if ($action === 'install') {
            ...
            PublicMailStorageService::saveSyncState(array(
                ...
                'title_normalization'=>array('enabled'=>false,'status'=>'disabled_for_speed','last_message'=>'평상시 메일 화면의 전체 제목 자동보정을 중지했습니다.')
            ));
            $indexedCount = isset($index['items']) && is_array($index['items']) ? count($index['items']) : 0;
            $result['message'] .= ' 메일 화면의 전체 제목 자동검사를 중지하고 빠른 목록 색인 ' . number_format($indexedCount) . '건을 다시 만들었습니다.';
...
$requiredFiles = array(
    ...
    'app/services/PublicMailStorageService.php' => "const VERSION = '1.7.12'",
    ...
    'app/services/PublicMailIndexService.php' => "const VERSION = '1.7.12'",
    'app/services/PublicMailService.php' => "const VERSION = '1.7.13'",
    'app/services/PublicMailWebHelper.php' => 'requireDevelopmentDepartment',
    'app/views/public_mail/index.php' => '20260806_713',
    'app/views/public_mail/detail_panel.php' => 'data-mail-detail-content',
    'app/views/public_mail/detail_fragment.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.13',
    'app/views/public_mail/settings.php' => '20260806_713',
    'public/public_mail.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.13',
    'public/public_mail_settings.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.13',
    'public/public_mail_action.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.12',
    'public/public_mail_attachment.php' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.13',
    'public/assets/css/public_mail.css' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.12',
    'public/assets/img/naver_n_icon.svg' => '',
    'public/assets/js/public_mail.js' => 'CPMS_PUBLIC_MAIL_VERSION: 1.7.13',
...
<div class="wrap">
    <div class="card">
        ...
        <p class="sub">v1.7.13 첨부파일 다운로드 수정 패치입니다. 숨겨진 iframe을 제거하고 일반 다운로드와 오류 안내를 정상 표시합니다.</p>
        ...
        <div class="status"><span class="dot <?php echo ($latestInstalled&&$allReady) ? 'on' : ''; ?>"></span><strong>설치 상태: <?php echo ($latestInstalled&&$allReady) ? 'v1.7.13 전체 적용' : ($installed ? '일부 파일 업데이트 필요' : '설치 전'); ?></strong></div><div class="status" style="margin-top:10px"><strong>목록 색인: <?php echo !empty($indexStatus['updated_at']) ? pm_install_h($indexStatus['updated_at']).' · '.number_format(isset($indexStatus['item_count'])?(int)$indexStatus['item_count']:0).'건' : '아직 생성되지 않음'; ?></strong></div>

        <div class="files">
            <?php foreach ($checks as $check): ?>
                <?php $checkClass = $check['status']==='latest'?'ok':($check['status']==='old'?'old':'bad'); ?>
                <?php $checkLabel = $check['status']==='latest'?'v1.7.13 구성 확인':($check['status']==='old'?'이전 버전':'파일 없음'); ?>
