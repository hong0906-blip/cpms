...
$canDecide = ($d && isset($d['doc_status']) && $d['doc_status'] === 'PENDING' && $myPendingLine);
...
<?php if($d && isset($d['doc_status']) && $d['doc_status']==='PENDING'){ ?>
<div class="no-print bg-white rounded-2xl border p-4 flex flex-wrap gap-3 items-center">
<?php if($canDecide){ ?>
<form method="post" action="?r=approval_decide" style="display:inline;"><input type="hidden" name="_csrf" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="id" value="<?php echo (int)$id; ?>"><input type="hidden" name="action" value="approve"><button type="submit" class="px-6 py-3 rounded-xl bg-emerald-600 text-white font-extrabold">승인하기</button></form>
<form method="post" action="?r=approval_decide" class="flex flex-wrap gap-2 items-center"><input type="hidden" name="_csrf" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="id" value="<?php echo (int)$id; ?>"><input type="hidden" name="action" value="reject"><input type="text" name="reject_reason" class="border rounded-xl px-3 py-2 min-w-[280px]" placeholder="반려사유 입력" required><button type="submit" class="px-6 py-3 rounded-xl bg-rose-600 text-white font-extrabold">반려하기</button></form>
<div class="text-sm text-gray-600">반려사유를 입력한 뒤 반려할 수 있습니다.</div>
<?php } else { ?>
