...
function render_approval_leave_document($data, $lines, $mode, $approvalOptions)
{
    ...
    if($mode==='edit'){ echo '<script>(function(){var s=document.querySelector("input[name=leave_start_date]");var e=document.querySelector("input[name=leave_end_date]");var d=document.querySelector("input[name=leave_days]");if(!s||!e||!d){return;}function c(){if(s.value===""||e.value===""){return;}var sv=new Date(s.value+"T00:00:00");var ev=new Date(e.value+"T00:00:00");if(ev.getTime()<sv.getTime()){alert("종료일은 시작일보다 빠를 수 없습니다.");d.value="";return;}var days=Math.floor((ev.getTime()-sv.getTime())/86400000)+1;d.value=days;}s.onchange=c;e.onchange=c;c();})();</script>'; }
