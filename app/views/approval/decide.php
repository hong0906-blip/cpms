...
if ($action === 'approve') {
    ...
    if($nextLine){
        ...
        approval_queue_notification($pdo,$id,'REQUEST',$nextLine['approver_id'],'[전자결재 요청]\n확인: ?r=approval_detail&id='.$id);        
    } else {
        $docStatus = 'APPROVED';
        $step = (int)$l['line_order'];
        $dst=$pdo->prepare("SELECT created_by_id FROM cpms_approval_documents WHERE id=:id"); $dst->execute(array(':id'=>$id)); $creatorId=(int)$dst->fetchColumn(); if($creatorId>0) approval_queue_notification($pdo,$id,'FINAL_APPROVED',$creatorId,'[전자결재 최종승인]\n확인: ?r=approval_detail&id='.$id);        
    }
    ...
    $dst=$pdo->prepare("SELECT created_by_id FROM cpms_approval_documents WHERE id=:id"); $dst->execute(array(':id'=>$id)); $creatorId=(int)$dst->fetchColumn(); if($creatorId>0) approval_queue_notification($pdo,$id,'REJECTED',$creatorId,'[전자결재 반려]\n확인: ?r=approval_detail&id='.$id);    
}
