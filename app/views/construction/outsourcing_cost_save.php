<?php
/** 파일: C:\www\cpms\app\views\construction\outsourcing_cost_save.php */
...
function cpms_outsourcing_cost_finish($isAjax, $ok, $message, $redirect, $entryId)
{
    if ($isAjax) {
        ...
        echo json_encode(array('ok'=>(bool)$ok,'message'=>(string)$message,'entry_id'=>(int)$entryId,'redirect'=>(string)$redirect));
        exit;
    }
...
if (!Auth::check()) { if ($isAjax) cpms_outsourcing_cost_finish(true,false,'로그인이 필요합니다.','?r=login',0); header('Location: ?r=login'); exit; }
if (!Auth::canManageConstruction()) { if ($isAjax) cpms_outsourcing_cost_finish(true,false,'저장 권한이 없습니다.','',0); http_response_code(403); echo '403 Forbidden'; exit; }
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); echo 'Method Not Allowed'; exit; }
$token=isset($_POST['_csrf'])?(string)$_POST['_csrf']:'';
if (!csrf_check($token)) cpms_outsourcing_cost_finish($isAjax,false,'보안 토큰이 유효하지 않습니다.','?r=공사',0);
...
try {
    ...
    if ($action==='delete') {
        ...
        $st->execute(array(':now'=>$now,':id'=>$entryId,':pid'=>$projectId));
        if (is_array($oldRow) && $st->rowCount()>0) {
            CostDataEventService::recordChange($pdo,array('project_id'=>$projectId,'cost_type'=>'outsourcing','target_type'=>'outsourcing_cost','target_id'=>(string)$entryId,'event_action'=>'DELETE','source_type'=>'DIRECT','actual_date'=>$oldRow['expense_date'],'settlement_ym'=>CostChangeService::settlementYm('outsourcing',$oldRow['expense_date']),'old_amount'=>$oldRow['amount'],'new_amount'=>null,'old_data'=>$oldRow,'new_data'=>array(),'source_file'=>__FILE__));
        }
        cpms_outsourcing_cost_finish($isAjax,true,'외주비 입력 내역을 삭제했습니다.',$redirect,$entryId);
    }
    ...
    cpms_outsourcing_cost_finish($isAjax,false,'외주비 저장 실패: '.$e->getMessage(),$redirect,$entryId);
}
