<?php
/**
 * C:\www\cpms\app\views\construction\issue_state_save.php
 * - 공사 기준 이슈 상태 변경 액션(POST)
 *
 * 변경 지점 주석:
 * - issue_update 경로 폐기
 * - issue_state_save 새 상태변경 action
 * - Apache 400 우회
...
function cpms_issue_state_save_column_exists($pdo, $table, $column)
{
    try {
...
$redirectUrl = cpms_issue_state_save_redirect_url($redirectKey, $projectId);
...
try {
    $hasUpdatedAt = cpms_issue_state_save_column_exists($pdo, 'cpms_project_issues', 'updated_at');
    if ($hasUpdatedAt) {
