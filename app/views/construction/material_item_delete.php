...
if ($projectId <= 0 || $materialId <= 0) { flash_set('error', '삭제 대상이 올바르지 않습니다.'); header('Location: ' . $redirect); exit; }
$pdo = Db::pdo();
if (!$pdo) { flash_set('error', 'DB 연결 실패'); header('Location: ' . $redirect); exit; }

try {
    $st = $pdo->prepare("UPDATE cpms_material_items SET is_deleted = 1, updated_at = :u WHERE id = :id AND project_id = :pid");
    $st->bindValue(':u', date('Y-m-d H:i:s'));
    $st->bindValue(':id', $materialId, PDO::PARAM_INT);
    $st->bindValue(':pid', $projectId, PDO::PARAM_INT);
    $st->execute();
    flash_set('success', '자재구입비를 삭제했습니다.');
} catch (Exception $e) {
