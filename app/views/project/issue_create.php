<?php
/**
 * C:\www\cpms\app\views\project\issue_create.php
 * - 이슈 등록 저장(POST)
 * - 등록자/날짜/시간 자동 저장
 *
 * PHP 5.6 호환
...
if ($reason === '') { flash_set('error','사유를 입력해주세요.'); header('Location: ?r=공무'); exit; }
if (mb_strlen($reason,'UTF-8') > 255) { $reason = mb_substr($reason,0,255,'UTF-8'); }

$pdo = Db::pdo();
if (!$pdo) { flash_set('error','DB 연결 실패'); header('Location: ?r=공무'); exit; }
