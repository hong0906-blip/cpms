...
$defaultRedirect = '?r=대시보드&dv=executive';
$redirect = isset($_POST['redirect']) && trim((string)$_POST['redirect']) !== '' ? trim((string)$_POST['redirect']) : $defaultRedirect;
if (strpos($redirect, '?r=') === false) $redirect = $defaultRedirect;
...
$pdo = Db::pdo();
if (!$pdo) {
    flash_set('error', 'ISSUE_UPDATE_LOADED=Y 이슈 상태 변경 실패: DB 연결에 실패했습니다.');
    header('Location: ' . $redirect);
    exit;
}
...
try {
    ...
    if (!$chk || !$chk->fetch()) {
        flash_set('error', 'ISSUE_UPDATE_LOADED=Y 이슈 상태 변경 실패: 이슈 테이블이 존재하지 않습니다.');
        header('Location: ' . $redirect);
        exit;
    }
    ...
    foreach ($needed as $col) {
        ...
        if (!$q || !$q->fetch()) {
            flash_set('error', 'ISSUE_UPDATE_LOADED=Y 이슈 상태 변경 실패: 필수 컬럼('.$col.')이 없습니다.');
            header('Location: ' . $redirect);
            exit;
        }
    }
