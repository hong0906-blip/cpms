<?php
/**
 * Lease overhead upload preview action.
 * PHP 5.6 compatible.
 */
...
$result = cpms_company_overhead_create_lease_preview($year, $month, $file, $user);

if (empty($result['ok'])) {
    flash_set('danger', isset($result['message']) ? (string)$result['message'] : '임대차 업로드 미리보기를 생성하지 못했습니다.');
    header('Location: ?r=' . urlencode('관리') . '&tab=company_overhead&oh=lease&year=' . urlencode((string)$year) . '&month=' . urlencode((string)$month));
    exit;
}

flash_set('success', '임대차 업로드 미리보기가 생성되었습니다. 확인 후 확정 저장해주세요.');
header('Location: ?r=' . urlencode('관리') . '&tab=company_overhead&oh=lease&year=' . urlencode((string)$result['year']) . '&month=' . urlencode((string)(int)$result['month']) . '&lease_preview_token=' . urlencode($result['token']));
exit;
