...
<div class="bg-gradient-to-r from-blue-600 to-cyan-500 rounded-3xl p-8 text-white shadow-xl shadow-blue-500/20 mb-8">
    <div class="flex items-start gap-4">
        ...
        <div class="flex flex-wrap gap-3"><!-- 직원 대시보드 UI 정리 -->
            <form method='post' action='?r=attendance/check_in'><input type='hidden' name='_csrf' value='<?php echo h(csrf_token());?>'><button class='px-5 py-3 rounded-2xl bg-white text-blue-700 font-extrabold text-base'>출근</button></form><form method='post' action='?r=attendance/check_out'><input type='hidden' name='_csrf' value='<?php echo h(csrf_token());?>'><button class='px-5 py-3 rounded-2xl bg-emerald-100 text-emerald-700 font-extrabold text-base'>퇴근</button></form><button type='button' data-attendance-request-open class='px-5 py-3 rounded-2xl bg-blue-900/80 text-white font-extrabold text-base border border-white/40'>출퇴근 요청</button></div>       
    </div>
</div>
