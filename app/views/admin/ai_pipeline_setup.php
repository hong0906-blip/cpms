    ...
    if (!csrf_check($token)) {
        ...
        try {
            if ($action === 'install') {
                ...
                $grace = isset($_POST['grace_days']) ? (int)$_POST['grace_days'] : 15;
                ...
                $ok = AiInputCompletionPatternService::saveSettings($apPdo, $grace, $minimum);
...
$apStatus = array('db_available'=>(bool)$apPdo, 'installed'=>false, 'latest_run'=>array(), 'run_count'=>0, 'settings'=>array('grace_days'=>15,'min_completion_rate'=>20));
...
<div class="ap-wrap">
 ...
 <?php if ($apResult): ?><div class="ap-message<?php echo empty($apResult['ok'])?' error':''; ?>"><?php echo h(isset($apResult['message'])?$apResult['message']:'요청을 처리했습니다.'); ?></div><?php endif; ?>
 ...
 <section class="ap-card" style="margin-top:16px"><h3>분석 설정</h3><form method="post" class="ap-form"><input type="hidden" name="_csrf" value="<?php echo h($apCsrf); ?>"><input type="hidden" name="action" value="save_settings"><label>입력 확정 유예일<input type="number" min="0" max="60" name="grace_days" value="<?php echo h($apStatus['settings']['grace_days']); ?>"></label><label>직접 투영 최소 완료율(%)<input type="number" min="1" max="100" name="min_completion_rate" value="<?php echo h($apStatus['settings']['min_completion_rate']); ?>"></label><label>GPT 질문 최소 간격(초)<input type="number" min="1" max="60" name="chat_min_seconds" value="<?php echo h($apChatStatus['min_seconds']); ?>"></label><label>GPT 시간당 질문 수<input type="number" min="1" max="200" name="chat_hourly_limit" value="<?php echo h($apChatStatus['hourly_limit']); ?>"></label><div><button class="ap-btn" type="submit">설정 저장</button></div></form><p class="ap-note">입력 확정은 해당 비용기간 종료 후 유예일이 지난 월만 사용합니다. 입력률 20% 미만 자료는 직접 투영 가중치를 낮춥니다. GPT는 저장된 근거자료만 설명하며 store=false로 호출됩니다.</p></section>
