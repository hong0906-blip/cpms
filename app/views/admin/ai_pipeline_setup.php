...
<div class="ap-wrap">
 ...
 <section class="ap-card" style="margin-top:16px"><h3>매일 19:00 크론 등록</h3><p class="ap-note">아래 설정은 요일 조건이 없으므로 토요일·일요일·공휴일을 포함해 매일 한국시간 19:00에 일일 스냅샷부터 CEO Index와 GPT 요약까지 순서대로 실행합니다. 먼저 설치 확인 명령을 한 번 실행한 뒤 <code>crontab -e</code>에 두 줄을 등록하세요.</p><code class="ap-code"><?php echo h($apSetupCommand); ?></code><code class="ap-code">CRON_TZ=Asia/Seoul
<?php echo h($apCronCommand); ?></code><p class="ap-note">PHP CLI 위치가 <code>/usr/bin/php</code>가 아니면 <code>command -v php</code> 결과로 바꾸세요. 실행 로그는 <code><?php echo h($apCronLogPath); ?></code>에 누적되며, 중복 호출은 파일 잠금과 DB 잠금으로 차단됩니다.</p></section>
