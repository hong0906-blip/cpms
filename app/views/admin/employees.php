...
<td class="px-4 py-3 font-bold"><?php echo h($r['name']); ?><div class="mt-2 text-xs space-y-1"><?php echo cpms_balance_badge('월차', isset($r['leave_monthly_balance'])?$r['leave_monthly_balance']:null); ?><br><?php echo cpms_balance_badge('연차', isset($r['leave_annual_balance'])?$r['leave_annual_balance']:null); ?><br><?php echo cpms_balance_badge('반차', isset($r['leave_half_balance'])?$r['leave_half_balance']:null); ?></div></td><td class="px-4 py-3"><?php echo h($r['email']); ?></td><td class="px-4 py-3"><?php echo h($r['department']); ?></td><td class="px-4 py-3"><?php echo h($r['hire_date'] ? $r['hire_date'] : '-'); ?></td><td class="px-4 py-3"><?php echo h($r['position']); ?></td><td class="px-4 py-3"><?php echo h($r['role']==='executive'?'임원':'직원'); ?></td><td class="px-4 py-3"><?php echo ((int)$r['is_active']===1)?'활성':'비활성'; ?></td>
