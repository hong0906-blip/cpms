...
if (!function_exists('cpms_employee_signature_preview_url')) {
function cpms_employee_signature_preview_url($path) {
    ...
    try {
        ...
        if (is_array($rows)) {
            for ($i = 0; $i < count($rows); $i++) {
                ...
                $rows[$i]['signature_preview_url'] = cpms_employee_signature_preview_url($rows[$i]['signature_effective_path']);
            }
        }
    } catch (\Exception $e) {
        $rows = array();
...
document.addEventListener('click', function(e) {
    ...
    if (be) {
        try {
            ...
            var signatureUrl = be.getAttribute('data-emp-signature-url') || '';
