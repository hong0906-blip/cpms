...
$approvalMemo = trim((string)(isset($_POST['approval_memo']) ? $_POST['approval_memo'] : ''));
$task = cpms_tasks_find_task($pdo, $taskId);

if (!$task || !cpms_tasks_can_approve_completion($task, $currentEmployeeId)) {
    flash_set('danger', '완료 승인 권한이 없습니다.');
    cpms_tasks_redirect_back();
}

try {
    if (cpms_tasks_approve_completion($pdo, $task, $currentEmployee, $approvalMemo, cpms_tasks_now())) {
        flash_set('success', '완료 승인 처리했습니다.');
    } else {
        flash_set('danger', '완료 승인 처리에 실패했습니다.');
    }
} catch (Exception $e) {
    flash_set('danger', '완료 승인 처리에 실패했습니다.');
}
