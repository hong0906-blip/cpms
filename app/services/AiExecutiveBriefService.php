    ...
    private static function buildV2SourceData($pdo)
    {
        ...
        try{
            ...
            foreach($rows as $row){
                ...
                $projects[]=array('project_id'=>$id,'project_name'=>$name,'project_status'=>self::redactText($row['project_status_snapshot'],50),'current_input_amount'=>(float)$row['current_input_amount'],'expected_completion_rate'=>$row['expected_completion_rate']===null?null:(float)$row['expected_completion_rate'],'expected_unentered_amount'=>(float)$row['expected_unentered_amount'],'final_forecast_amount'=>$forecast,'forecast_low_amount'=>(float)$row['forecast_low_amount'],'forecast_high_amount'=>(float)$row['forecast_high_amount'],'forecast_confidence_grade'=>(string)$row['forecast_confidence_grade'],'overinput_grade'=>(string)$row['overinput_grade'],'missing_possibility_grade'=>(string)$row['missing_possibility_grade'],'contract_risk_grade'=>(string)$row['contract_risk_grade'],'anomaly_count'=>(int)$row['anomaly_count'],'project_index_score'=>$row['project_index_score']===null?null:(float)$row['project_index_score'],'project_index_grade'=>(string)$row['project_index_grade'],'monthly_sales_amount'=>$sales,'monthly_forecast_profit_amount'=>$profit,'evidence_ids'=>$evidenceIds,'evidence'=>$projectEvidence);
            }
